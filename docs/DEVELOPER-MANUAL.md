...
| `core/timeline.py`, `core/weekgrid.py`, `ui/views/` | FR-509: `layout_day()` (minutes, lanes for overlaps, gaps, totals — pure) painted by `DayCanvas`; FR-510: `build_week_grid()` over `EntryRepo.totals_by_client_and_day` (one grouped scan) shown by `WeekView`; `ViewsWindow` hosts both tabs, opened from the log toolbar |
